...
const SettingsModal = ({ 
  ...
  const [activeTab, setActiveTab] = useState('analytics'); // 'analytics' | 'categories'
      ...
      <div className="relative w-full max-w-2xl bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden my-8">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-800 bg-slate-900/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white">Store Settings & Control</h2>
              <p className="text-xs text-slate-400">Manage shop analytics, metrics & product categories</p>
            </div>
          </div>
        ...
        <div className="p-6 max-h-[70vh] overflow-y-auto space-y-6">
          ...
          {/* TAB 1: INVENTORY ANALYTICS & METRICS */}
          {activeTab === 'analytics' && (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                >
                  <div className="flex items-center justify-between">
                    <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Unique Parts</p>
                    <div className="p-2 bg-blue-500/10 text-blue-400 rounded-lg border border-blue-500/20">
                      <Package className="w-4 h-4" />
                    </div>
                  </div>
                  <h3 className="text-2xl font-extrabold text-white mt-2">{totalItems}</h3>
                  <p className="text-[10px] text-slate-500 mt-1">Active catalog parts</p>
                </div>
                ...
                <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800">
                  ...
                  <h3 className="text-2xl font-extrabold text-white mt-2">{totalQuantity} <span className="text-xs font-normal text-slate-400">units</span></h3>
                  <p className="text-[10px] text-slate-500 mt-1">In-stock physical units</p>
                </div>
                ...
                <div 
                  ...
                  className={`p-4 rounded-2xl border transition-all duration-200 cursor-pointer ${
                    filterLowStock 
                      ? 'bg-amber-950/40 border-amber-500/50 ring-2 ring-amber-500/40' 
                      : 'bg-slate-950 border-amber-500/30 hover:border-amber-500/60'
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <p className="text-[10px] font-bold uppercase tracking-wider text-amber-400">Low Stock (&lt;5)</p>
                    <div className="p-2 bg-amber-500/10 text-amber-400 rounded-lg border border-amber-500/20">
                      <AlertTriangle className="w-4 h-4" />
                    </div>
                  </div>
                  <h3 className="text-2xl font-extrabold text-amber-400 mt-2">{lowStockCount}</h3>
                  <div className="flex items-center justify-between mt-1 text-[10px] text-amber-400 font-semibold">
                    <span>{filterLowStock ? 'Clear Filter' : 'Filter Low Stock'}</span>
